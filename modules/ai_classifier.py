...
class AIClassifier:
    def __init__(self):
        """Initialize the classifier with Ollama LLM backend support."""
        self.company_roles = [
    ...
    def _load_llm_config(self) -> Dict[str, Any]:
        """Load Ollama LLM configuration from Streamlit secrets."""
        config = {
            'ollama': {
                'base_url': st.secrets.get('OLLAMA_BASE_URL', 'http://localhost:11434'),
                'model': st.secrets.get('OLLAMA_MODEL', 'llama3.1:8b-instruct-q4_K_M'),
                'timeout': 30
            }
        }
        return config
...
JSON Response:"""
    ...
    def extract_info(self, email_subject: str, email_body: str, resume_text: str) -> Optional[Dict[str, Any]]:
        """Extract structured data using the Ollama LLM."""
        try:
            combined_text = (
            ...
            result = self._extract_with_ollama(combined_text)
            
            if result and self._validate_extraction(result):
                logger.info("Successfully extracted data using Ollama")
                return self._post_process_result(result)
            else:
                logger.warning("Ollama extraction failed or returned invalid data")
                return None
    ...
    def _extract_with_ollama(self, combined_text: str) -> Optional[Dict[str, Any]]:
        """Extract using a local Ollama instance."""
        ...
        try:
            ...
            logger.error(f"Connection Error: Could not connect to Ollama at {config['base_url']}. Is Ollama running?")
            return None
        except Exception as e:
            logger.error(f"Error with Ollama backend: {str(e)}")
            return None
